...
stan <- stan_glm(data = covid_results,
                 formula = trump_pct ~ pct_degree + unemployment_rate_chg_2000_2019 + state.x -1,
                 refresh = 0)
stan
...
covid_results %>%
#  filter(state.x == c("michigan", "pennsylvania", "wisconsin", "arizona", "georgia")) %>%
  ggplot(mapping = aes(x = cases_per_10000, y = trump_pct, size = population, color = state)) +
